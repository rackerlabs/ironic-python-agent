...
metrics_opts = [
    cfg.StrOpt('backend',
               default='noop',
               help='Backend to use for the metrics system. Default included '
                    'backends are "noop" and "statsd".'),
    cfg.BoolOpt('prepend_host',
                default=False,
                help='Prepend the value of CONF.host to all metric names. The'
                     'format of the metrics prefix is <global_prefix>.<uuid>.'
                     '<host>'),
    cfg.BoolOpt('prepend_host_reverse',
                default=True,
                help='Split the prepended host value by the delimiter and '
                     'reverse it (to better match the reverse hierarchical '
                     'form of domain names).'),
    cfg.BoolOpt('prepend_uuid',
                default=False,
                help='Prepend the node\'s Ironic UUID to all metric names. '
                     'The format of the metrics prefix is <global_prefix>.'
                     '<uuid>.<host>'),
    cfg.StrOpt('global_prefix',
               default='',
               help='Prefix all metric names with this value. The format of '
                    'the metrics prefix is <global_prefix>.<uuid>.<host>.'),
...
class InvalidMetricsConfig(ValueError):
    pass
...
class MetricLogger(object):
    ...
    def counter(self, m_parts, m_value, sample_rate=None):
        ...
        if sample_rate is None or random.random() < sample_rate:
            return self._counter(self._getMetricName(m_parts),
    ...
    def instrument(self, *parts):
        """Returns a decorator to instrument with this MetricsLogger
...
class StatsdMetricLogger(MetricLogger):
    ...
    def __init__(self, prefix_parts, delimiter,
        ...
        """Initialize a StatsdMetricLogger with the given prefixes."""
    ...
    def _send(self, m_name, m_value, m_type, sample_rate=None):
        ...
        LOG.debug('statsd: sending %(metric)s to %(target)s',
...
def getLogger(prefix):
    ...
    # TODO(JoshNang) make this actually pluggable
    if CONF.metrics.backend == 'statsd':
        return StatsdMetricLogger(prefix, '.',
                                  host=CONF.metrics.statsd_host,
                                  port=CONF.metrics.statsd_port)
    elif CONF.metrics.backend == 'noop':
        return NoopMetricLogger(prefix, '.')
    else:
        raise InvalidMetricsConfig('Backend %s is not a supported metrics '
                                   'backend.' % CONF.metrics.backend)
